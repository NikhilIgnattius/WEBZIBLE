...
export default function changeAlignment() {
  document.querySelector("#alignmentChange .tick-check").style.display = "block";
  document.querySelector("#alignmentChange .option-total").style.display = "flex";
  currentIndex = (currentIndex + 1) % alignments.length;
  currentIcon = iconList[currentIndex % alignments.length];
  if (clickCount <= 4) {
    document
      .querySelector(`#alignmentChange .option${clickCount}`)
...
function resetAlignment() {
  document.querySelector("#alignmentChange .tick-check").style.display = "none";
    document.querySelector("#alignmentChange .option-total").style.display = "none";
    for (let index = 1; index < 5; index++) {
      document.querySelector(`#alignmentChange .option${index}`).classList.remove("selected");
    }
